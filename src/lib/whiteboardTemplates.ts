...
const firstPrinciples: BoardTemplate = {
  ...
  build: () => [
    t(40, 28, 'PROBLEM (one sentence)', { size: 1.8, color: ACCENT }),
    box(36, 52, 720, 56),
    t(48, 70, '…', { size: 2.4, color: INK }),

    t(40, 130, '1. WHAT IS TRUE?  (facts only — no opinions)', { size: 1.8, color: ACCENT }),
    box(36, 154, 340, 200),
    t(48, 170, '•', { color: INK }),
    t(48, 200, '•', { color: INK }),
    t(48, 230, '•', { color: INK }),
    ...
    t(400, 130, '2. CONSTRAINTS  (time, people, GxP, cost)', { size: 1.8, color: ACCENT }),
    box(396, 154, 360, 200),
    t(408, 170, '•', { color: INK }),
    t(408, 200, '•', { color: INK }),
    t(408, 230, '•', { color: INK }),

    t(40, 380, '3. FASTEST PATH FROM TRUTH → OUTCOME', { size: 1.8, color: ACCENT }),
    box(36, 404, 720, 100),
    t(48, 420, '1.', { color: INK }),
    t(48, 450, '2.', { color: INK }),
    t(48, 480, '3.', { color: INK }),
  ],
};
...
const blockers: BoardTemplate = {
  ...
  build: () => [
    t(40, 28, 'BLOCKED WORK', { size: 1.8, color: '#b91c1c' }),
    ...
    t(420, 28, 'NEXT MOVE  ·  OWNER  ·  BY WHEN', { size: 1.8, color: ACCENT }),
    box(416, 52, 340, 280),
    t(428, 70, '→', { color: INK }),
    t(428, 120, '→', { color: INK }),
    t(428, 170, '→', { color: INK }),
    ...
    t(40, 360, 'ESCALATE IF STILL STUCK AFTER', { size: 1.8, color: MUTED }),
    box(36, 384, 720, 72),
  ],
};
...
const decide: BoardTemplate = {
  ...
  build: () => [
    t(40, 28, 'DECISION', { size: 1.8, color: ACCENT }),
    box(36, 52, 720, 48),
    t(48, 66, 'We will…', { color: INK }),

    t(40, 120, 'OPTION A', { size: 1.8, color: ACCENT }),
    box(36, 144, 340, 180),
    t(48, 160, 'Pros', { size: 1.6, color: MUTED }),
    t(48, 220, 'Cons', { size: 1.6, color: MUTED }),

    t(400, 120, 'OPTION B', { size: 1.8, color: ACCENT }),
    box(396, 144, 360, 180),
    t(408, 160, 'Pros', { size: 1.6, color: MUTED }),
    t(408, 220, 'Cons', { size: 1.6, color: MUTED }),
...
const delivery: BoardTemplate = {
  ...
  build: () => [
    t(40, 28, 'NOW  (this week — ship or fail)', { size: 1.8, color: '#b91c1c' }),
    box(36, 52, 230, 320, '#fca5a5'),
    t(48, 70, '1.', { color: INK }),
    t(48, 120, '2.', { color: INK }),
    t(48, 170, '3.', { color: INK }),

    t(290, 28, 'NEXT  (after NOW is done)', { size: 1.8, color: ACCENT }),
    box(286, 52, 230, 320),
    t(298, 70, '1.', { color: INK }),
    t(298, 120, '2.', { color: INK }),
    t(298, 170, '3.', { color: INK }),

    t(540, 28, 'LATER  (parked, not forgotten)', { size: 1.8, color: MUTED }),
    ...
    t(40, 400, 'Kill list — work we will NOT do:', { size: 1.8, color: MUTED }),
    box(36, 424, 720, 64),
  ],
};
